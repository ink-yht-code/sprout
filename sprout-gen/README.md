...
cd sprout
go build -o sprout-gen ./sprout-gen
```
...
```bash
# 创建新服务骨架（HTTP）
sprout-gen new service myservice --transport http --module github.com/your-org/your-repo/myservice
...
```bash
sprout-gen new service <name> [flags]
```
...
- `--registry, -r` - Registry 服务地址（`none` 表示不启用）
- `--module` - Go module path（例如 `github.com/xxx/project/service`；为空则使用服务名）
